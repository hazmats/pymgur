...
def get_all_comments(target):
  ...
  #rsptimes = []
  while getmore == True:
    req = requests.get('http://'+host+'/user/'+target+'/index/newest/page/'+str(page))
    #rsptimes.append(req.elapsed)
    csoup = BeautifulSoup(req.text)
    page += 1
    found = 0
    # Collect each comment from this page
    for x in csoup.find_all('div', class_='caption'):
       cmnts.append(x['data'])
       found += 1
...
if args.source == None:
   # No source user passed in, attempting to read from multi-user
   # and step through the process for each source user.
   sources = {}
   print "No source user defined, attempting to use creds in %s" % userfile
   for x in open(userfile):
      sources[x.split(";")[0]] = x.split(";")[1]
   cmnts = get_all_comments(args.target)
   for x in sources:
      session = requests.Session()
      print "Starting session as %s." % x
      login(x, sources[x])
      vote(cmnts, args.action, 'catption')
      session.close()
